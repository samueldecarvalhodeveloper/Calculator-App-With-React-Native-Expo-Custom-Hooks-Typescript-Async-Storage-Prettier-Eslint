import { describe, test, expect } from "@jest/globals";
import ThemeStore from "../../../user_interface/store/theme_store";
import Themes from "../../../assets/themes";
import KeyValueDatabase from "../../../infrastructure/adapters/key_value_database";
import { THEME_STORE_KEY } from "../../../constants/user_interface_constants";

describe('Test Class: "ThemeStore"; Behavior', () => {
  test('Test If Method: "setTheme"; Updates Theme Key In Key Value Database', async () => {
    ThemeStore.setTheme(Themes.DARK);

    const themeFromKeyValueDatabase: string = (await KeyValueDatabase.getItem(
      THEME_STORE_KEY,
    )) as string;

    expect(themeFromKeyValueDatabase).toEqual(Themes.DARK);
  });

  test('Test If Method: "getTheme"; Returns The Theme Stored On Key Value Database', async () => {
    KeyValueDatabase.setItem(THEME_STORE_KEY, Themes.LIGHT);

    const theme = await ThemeStore.getTheme();

    expect(theme).toEqual(Themes.LIGHT);
  });
});
